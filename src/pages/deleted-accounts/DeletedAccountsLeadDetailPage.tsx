...
import {
  useAdminDeletedSalesActivities,
  useAdminDeletedSalesLeadDetail,
  useAssignDeletedSalesLead,
  useClaimDeletedSalesLead,
  useLogDeletedSalesCommunication,
  useReleaseDeletedSalesLead,
  useUpdateDeletedSalesFollowUp,
  useUpdateDeletedSalesNote,
  useUpdateDeletedSalesStatus,
...
type SalesDetailLocationState = {
  deletedAccountsListSearch?: string
}

function deletedAccountsListSearchForLink(raw?: string): string {
  if (!raw) return ''
...
export default function DeletedAccountsLeadDetailPage() {
  const { userId } = useParams<{ userId: string }>()
  const location = useLocation()
  const { session } = useAuth()
  const state = location.state as SalesDetailLocationState | null
  const backSearch = deletedAccountsListSearchForLink(state?.deletedAccountsListSearch)

  const query = useAdminDeletedSalesLeadDetail(userId)
  const activitiesQuery = useAdminDeletedSalesActivities(userId, { page: 0, size: 20 })
  const updateStatus = useUpdateDeletedSalesStatus(userId ?? '')
  const updateNote = useUpdateDeletedSalesNote(userId ?? '')
  const updateFollowUp = useUpdateDeletedSalesFollowUp(userId ?? '')
  const claimLead = useClaimDeletedSalesLead(userId ?? '')
  const releaseLead = useReleaseDeletedSalesLead(userId ?? '')
  const assignLead = useAssignDeletedSalesLead(userId ?? '')
  const logCommunication = useLogDeletedSalesCommunication(userId ?? '')
  ...
  return (
    <section className="min-w-0 space-y-4">
      <PageHeader title="Deleted Account Detail" description={`Manage deleted sales lead for ${userId ?? '--'}.`} />
      <Link to={{ pathname: routes.deletedAccounts, search: backSearch }} className="text-sm text-slate-700 underline">
        Back to sales
      </Link>
            ...
            <div className="min-w-0 flex-1 space-y-4">
              <Card>
                ...
                <CardContent className="space-y-2 text-sm">
                  ...
                  <Row label="Deletion at" value={formatDateTime(query.data.deletionAt)} />
                  <Row label="Soft deleted at" value={formatDateTime(query.data.softDeletedAt)} />
                  <Row label="Purged at" value={formatDateTime(query.data.purgedAt)} />
