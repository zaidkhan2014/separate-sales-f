...
import { useAdminSalesFollowUps } from '@/hooks/api/useAdminSales'
...
export default function SalesFollowUpsPage() {
  ...
  const query = useAdminSalesFollowUps(filters)
  const totalPages = Math.max(1, Math.ceil((query.data?.total ?? 0) / PAGE_SIZE))

  return (
    <section className="min-w-0 space-y-4">
      <PageHeader title="Sales Follow-ups" description="Follow-up queue by UTC day bucket." />
              ...
              <tbody>
                {query.data.items.map((lead) => (
                  <tr key={lead.userId} className="border-t border-slate-100">
                    ...
                    <td className="px-3 py-2">
                      <Link className="underline" to={routes.leadDetail(lead.userId)}>
                        Open
                      </Link>
                    </td>
                  </tr>
